...
export const WithdrawUSDC: FC<WithdrawUSDCProps> = (props) => {
  ...
  const onClick = useCallback(async () => {
    ...
    const [userConfigPk, butlerUserConfigBump] =
      await anchor.web3.PublicKey.findProgramAddress(
        [
    ...
    try {
      const withdrawDriftCollateralIx =
        await program.instruction.withdrawDriftCollateral(
          stateBump,
          accountOwnerBump,
          butlerDriftCollateralBump,
          butlerMangoCollateralBump,
          butlerUserConfigBump,
          driftQuantityNative,
          false,
          {
            accounts: {
      ...
      const withdrawIx = program.instruction.withdrawMangoCollateral(
        stateBump,
        mangoAccountBump,
        accountOwnerBump,
        butlerMangoCollateralBump,
  ...
  return (
    <div>
      ...
      >
        <span>
          {' '}
          Withdraw {(mangoWithdrawValue + driftWithdrawValue).toFixed(0)} USDC
        </span>
      </button>
    </div>
  );
